...
func (dq *DeviceQuery) QueryGpu() *GpuQuery {
	query := (&GpuClient{config: dq.config}).Query()
...
func (dq *DeviceQuery) WithGpu(opts ...func(*GpuQuery)) *DeviceQuery {
	query := (&GpuClient{config: dq.config}).Query()
...
func (dq *DeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Device, error) {
	...
	if query := dq.withGpu; query != nil {
		if err := dq.loadGpu(ctx, query, nodes,
			func(n *Device) { n.Edges.Gpu = []*Gpu{} },
			func(n *Device, e *Gpu) { n.Edges.Gpu = append(n.Edges.Gpu, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeviceQuery) loadGpu(ctx context.Context, query *GpuQuery, nodes []*Device, init func(*Device), assign func(*Device, *Gpu)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(gpu.FieldDeviceID)
	}
	query.Where(predicate.Gpu(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(device.GpuColumn), fks...))
	}))
