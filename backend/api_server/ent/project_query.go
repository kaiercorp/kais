...
type ProjectQuery struct {
	...
	withTasks       *TaskQuery
	withUserProject *UserProjectQuery
...
// QueryUserProject chains the current query on the "user_project" edge.
func (pq *ProjectQuery) QueryUserProject() *UserProjectQuery {
	query := (&UserProjectClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withUserProject: pq.withUserProject.Clone(),
...
// WithUserProject tells the query-builder to eager-load the nodes that are connected to
// the "user_project" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithUserProject(opts ...func(*UserProjectQuery)) *ProjectQuery {
	query := (&UserProjectClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserProject = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withUserProject; query != nil {
		if err := pq.loadUserProject(ctx, query, nodes,
			func(n *Project) { n.Edges.UserProject = []*UserProject{} },
			func(n *Project, e *UserProject) { n.Edges.UserProject = append(n.Edges.UserProject, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadUserProject(ctx context.Context, query *UserProjectQuery, nodes []*Project, init func(*Project), assign func(*Project, *UserProject)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.UserProject(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.UserProjectColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
