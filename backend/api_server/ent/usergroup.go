...
func (*UserGroup) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usergroup.FieldID, usergroup.FieldLevel:
...
func (ug *UserGroup) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", ug.Level))
	builder.WriteString(", ")
	builder.WriteString("is_use=")
	builder.WriteString(fmt.Sprintf("%v", ug.IsUse))
	builder.WriteByte(')')
