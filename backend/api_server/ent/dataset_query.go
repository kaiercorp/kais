...
type DatasetQuery struct {
	...
	withDatasetroot *DatasetRootQuery
	withFKs         bool
...
// QueryDatasetroot chains the current query on the "datasetroot" edge.
func (dq *DatasetQuery) QueryDatasetroot() *DatasetRootQuery {
	query := (&DatasetRootClient{config: dq.config}).Query()
...
// WithDatasetroot tells the query-builder to eager-load the nodes that are connected to
// the "datasetroot" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DatasetQuery) WithDatasetroot(opts ...func(*DatasetRootQuery)) *DatasetQuery {
	query := (&DatasetRootClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDatasetroot = query
	return dq
}
...
func (dq *DatasetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Dataset, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [1]bool{
			dq.withDatasetroot != nil,
		}
	)
	if dq.withDatasetroot != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withDatasetroot; query != nil {
		if err := dq.loadDatasetroot(ctx, query, nodes, nil,
			func(n *Dataset, e *DatasetRoot) { n.Edges.Datasetroot = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DatasetQuery) loadDatasetroot(ctx context.Context, query *DatasetRootQuery, nodes []*Dataset, init func(*Dataset), assign func(*Dataset, *DatasetRoot)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Dataset)
	for i := range nodes {
		if nodes[i].dataset_root_datasets == nil {
			continue
		}
		fk := *nodes[i].dataset_root_datasets
	...
	query.Where(datasetroot.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "dataset_root_datasets" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
