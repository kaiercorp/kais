...
// QueryModelings chains the current query on the "modelings" edge.
func (tq *TaskQuery) QueryModelings() *ModelingQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withModelings: tq.withModelings.Clone(),
...
// WithModelings tells the query-builder to eager-load the nodes that are connected to
// the "modelings" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithModelings(opts ...func(*ModelingQuery)) *TaskQuery {
	query := (&ModelingClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withModelings = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withModelings; query != nil {
		if err := tq.loadModelings(ctx, query, nodes,
			func(n *Task) { n.Edges.Modelings = []*Modeling{} },
			func(n *Task, e *Modeling) { n.Edges.Modelings = append(n.Edges.Modelings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadModelings(ctx context.Context, query *ModelingQuery, nodes []*Task, init func(*Task), assign func(*Task, *Modeling)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(modeling.FieldTaskID)
	}
	query.Where(predicate.Modeling(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ModelingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
