...
import { BaseModalTitleType, compareTrials, emptyBaseModalTitle, filterTrials, tableClsAdditionalTrain, tableClsAutoTrain, tableClsMultiTest, tableClsSingleTest, emptyTableClsRequest } from 'common'
import TrainTableCls from './TrainTableCls'
import TrainTableClsAdditional from './TrainTableClsAdditional'

const TableCLSLanding = () => {
  ...
  const projects = queryClient.getQueryData<ProjectType[]>([`${QUERY_KEY.projects}_table.cls`])
  ...
  useSocket('/trials/' + location.pathname.split('/').pop(), 'Trials', handleSocketMessage, { shouldCleanup: true, shouldConnect: true })

  useEffect(() => {
    logger.log(`Change Location to ${t('title.table.cls.trials')}`)
    updateLocationContextValue({ location: 'table.cls.trials' })
  ...
  const openAutoTrainModal = () => {
    updateTrialContextValue({ trainMode: 'auto', requestData: objDeepCopy(emptyTableClsRequest) })

    setModalTitle(tableClsAutoTrain)
    setModalBody(<TrainTableCls ref={childComponentRef} />)
    openModal()
  }

  const openMultiTestModal = () => {
    setModalTitle(tableClsMultiTest)
    setModalBody(<MultiSampleTest ref={childComponentRef} engineType={engine.table_cls}/>)
    openModal()
  }

  const openSingleTestModal = () => {
    setModalTitle(tableClsSingleTest)
    setModalBody(<SingleSampleTest ref={childComponentRef} engineType={engine.table_cls}/>)
  ...
  const openAdditionalTrainModal = (row: any) => {
    updateTrialContextValue({ trainMode: 'additional', requestData: objDeepCopy(emptyTableClsRequest) })

    setModalTitle(tableClsAdditionalTrain)
    setModalBody(<TrainTableClsAdditional trialId={row.trial_id} ref={childComponentRef} />)
    openModal()
  }
...
export default TableCLSLanding
