...
import { RegModelList } from 'features/TrainModel'
import { TableClsTrialConfigsTable, HyperParameterHistory, LogHistory } from 'features/TrialDetail'
import { useToggle, useSocket } from 'hooks'
import { useTranslation } from 'react-i18next'
import { logger, ApiFetchTrial, ApiDownloadFile } from 'helpers'
import { TSADTrainContainer } from 'features/TrialDetail'
import { BaseModalTitleType, emptyBaseModalTitle, tableRegMultiTest, TrainType } from 'common'
...
const TSADTTrainDetail = () => {
    ...
    const [trains, setTrains] = useState<TrainType[] | undefined>()
    ...
    useEffect(() => {
        ...
        logger.log(`Change Location to ${t('title.ts.ad.train', { trial: trial.trial_name })}`)
        updateLocationContextValue({ location: 'ts.ad.train', locationValue: { trial: `[${trial.trial_id}] ${trial.trial_name}` } })
    ...
    // const [wsTrainConnected, setWsTrainConnected] = useState(false)
    ...
    const openMultiTestModal = (row: any) => {
        ...
        setModalBody(<TSADMultiSampleTest ref={childComponentRef} selectedTrial={trial} selectedModels={row} />)
        openModal()
    }
                            ...
                            <Tab.Container defaultActiveKey='tabs-summary'>
                                ...
                                <Tab.Content>
                                    <Tab.Pane eventKey='tabs-summary' id='tabs-summary'>
                                        {trial && trial.trial_id > 0 && <TSADTrainContainer trial={trial} trains={trains} />}
                                    </Tab.Pane>
...
export default TSADTTrainDetail