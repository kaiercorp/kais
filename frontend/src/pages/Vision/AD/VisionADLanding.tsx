...
import { BaseModalTitleType, compareTrials, emptyBaseModalTitle, filterTrials, visionADAdditionalTrain, visionADAutoTrain, visionADMultiTest, visionADSingleTest, ProjectType, emptyVADTrial } from 'common'
import { CompareTrial, Train, TrainAdditional } from 'pages/Vision'

const VisionADLanding = () => {
  ...
  const projects = queryClient.getQueryData<ProjectType[]>([`${QUERY_KEY.projects}_vision.ad`])
  ...
  useEffect(() => {
    logger.log(`Change Location to ${t('title.vision.ad.trials')}`)
    updateLocationContextValue({ location: 'vision.ad.trials' })
  ...
  const openAutoTrainModal = () => {
    updateTrialContextValue({ trainMode: 'auto', requestData: objDeepCopy(emptyVADTrial) })

    setModalTitle(visionADAutoTrain)
    setModalBody(<Train ref={childComponentRef} engineType={engine.vision_ad}/>)
    openModal()
  }

  const openMultiTestModal = () => {
    setModalTitle(visionADMultiTest)
    setModalBody(<FolderTest ref={childComponentRef} engineType={engine.vision_ad} />)
    openModal()
  }

  const openSingleTestModal = () => {
    setModalTitle(visionADSingleTest)
    setModalBody(<FileTest ref={childComponentRef} engineType={engine.vision_ad} />)
  ...
  const openAdditionalTrainModal = (row: any) => {
    updateTrialContextValue({ trainMode: 'additional', requestData: objDeepCopy(emptyVADTrial) })

    setModalTitle(visionADAdditionalTrain)
    setModalBody(<TrainAdditional trialId={row.trial_id} ref={childComponentRef} engineType={engine.vision_ad}/>)
    openModal()
  }
...
export default VisionADLanding
