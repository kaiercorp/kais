...
import { TestModelCard, ModelsArea, TrainModelContainer, MLTestDataResult } from 'features/TrainModel'
...
const VisionCLSTestDetail = () => {
    ...
    const onSelectData = (data_index: any) => {
        if (!socketConnected || !ws || !ws.current) return
        ws.current.send(JSON.stringify({ sample_id: data_index, engine_type: engine.vision_cls_ml }))
    }
                        ...
                        <Card.Body>
                            {
                                selectedModel && (
                                    <Row style={{ marginBottom: '10px' }}>
                                        ...
                                        <Col sm={10}>
                                            <MLMatrix ml_matrixStr={selectedModel.ml_matrix.String} />
                                        </Col>
                                    </Row>
                                )
                            }
                            <Row>
                                <Col>
                                    {resultList && <MLTestDataResult resultList={resultList} onClick={onSelectData} />}
                                </Col>
                            </Row>
                        </Card.Body>
                    </Card>
...
export default VisionCLSTestDetail