...
import { BaseModalTitleType, compareTrials, emptyBaseModalTitle, filterTrials, visionClsAdditionalTrain, visionClsMLAutoTrain, visionClsMultiTest, visionClsSingleTest, emptyMLClsTrial, ProjectType } from 'common'
...
const VisionCLSLanding = () => {
  ...
  const projects = queryClient.getQueryData<ProjectType[]>([`${QUERY_KEY.projects}_vision.cls-ml`])
  ...
  useEffect(() => {
    logger.log(`Change Location to ${t('title.vision.cls-ml.trials')}`)
    updateLocationContextValue({ location: 'vision.cls-ml.trials' })
  ...
  const openAutoTrainModal = () => {
    updateTrialContextValue({ trainMode: 'auto', requestData: objDeepCopy(emptyMLClsTrial) })

    setModalTitle(visionClsMLAutoTrain)
    setModalBody(<Train ref={childComponentRef} engineType={engine.vision_cls_ml}/>)
    openModal()
  }

  const openMultiTestModal = () => {
    setModalTitle(visionClsMultiTest)
    setModalBody(<FolderTest ref={childComponentRef} engineType={engine.vision_cls_ml}/>)
    openModal()
  }

  const openSingleTestModal = () => {
    setModalTitle(visionClsSingleTest)
    setModalBody(<FileTest ref={childComponentRef} engineType={engine.vision_cls_ml}/>)
  ...
  const openAdditionalTrainModal = (row: any) => {
    updateTrialContextValue({ trainMode: 'additional', requestData: objDeepCopy(emptyMLClsTrial) })

    setModalTitle(visionClsAdditionalTrain)
    setModalBody(<TrainAdditional trialId={row.trial_id} ref={childComponentRef} engineType={engine.vision_cls_ml}/>)
    openModal()
  }
    ...
    <>
      ...
      <Row style={{ marginTop: '5px' }}>
        <TrialTable
          ...
          accCol={t('ui.formatter.label_accuracy')}
        />
      </Row>
