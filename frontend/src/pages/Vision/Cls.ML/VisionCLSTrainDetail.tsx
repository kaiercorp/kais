...
import { CLSModelList, 
    ...
    FolderTest,
    BaseModal,
    VisionClsTrialConfigsTable,
    HyperParameterHistory,
    LogHistory } from 'features'
import { useToggle, useSocket } from 'hooks'
import { useTranslation } from 'react-i18next'
import { logger, ApiFetchTrial, ApiDownloadFile } from 'helpers'
import { BaseModalTitleType, emptyBaseModalTitle, visionClsMultiTest } from 'common'
...
const VisionCLSTrainDetail = () => {
    ...
    useEffect(() => {
        if (!trial || !trial.project_id) return

        logger.log(`Change Location to ${t('title.vision.cls.train', { trial: trial.trial_name })}`)
        updateLocationContextValue({location: 'vision.cls.train', locationValue: { trial: `[${trial.trial_id}] ${trial.trial_name}` }})
    ...
    useSocket(`/trials/trains/${trial?.trial_id}`, 'Trains', handleSocketMessage, {shouldCleanup: true, shouldConnect: !!trial && trial.trial_id !== 0})
    ...
    const openMultiTestModal = (row: any) => {
        ...
        setModalBody(<FolderTest ref={childComponentRef} selectedTrial={trial} selectedModels={row} engineType={engine.vision_cls_ml}/>)
        openModal()
    }
                    ...
                    <Col>
                        <Card>
                            <Card.Header>
                                ...
                                {trial && trial.state === 'finish' && <CardHeaderButton onClick={openMultiTestModal}>{t('modal.title.vision.cls.multitest.title')}</CardHeaderButton>}
                                {user && <CardHeaderButton variant='success' onClick={handleDownloadReport}>Save Report</CardHeaderButton>}
                            </Card.Header>
                            <Card.Body>
                                <VisionClsTrialConfigsTable trial={trial} />
                            </Card.Body>
                        </Card>
                    </Col>
                </Row>
                            ...
                            <Tab.Container defaultActiveKey='tabs-summary'>
                                ...
                                <Tab.Content>
                                    <Tab.Pane eventKey='tabs-summary' id='tabs-summary'>
                                        {trial && trial.trial_id > 0 && <VisionCLSTrainChart trial={trial} trains={trains} />}
                                    </Tab.Pane>
                                    <Tab.Pane eventKey='tabs-models' id='tabs-models'>
                                        {eventKey === 'tabs-models' && trial && trial.trial_id > 0 && <CLSModelList trial={trial} openMultiTest={openMultiTestModal} downloadModel={handleDownloadModel} engineType={engine.vision_cls_ml}/>}
                                    </Tab.Pane>
...
export default VisionCLSTrainDetail