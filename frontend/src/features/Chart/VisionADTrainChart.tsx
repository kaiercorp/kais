...
const refineChartData = (chartData: any, chartRefAccuracy: any) => {
...
const VisionADTrainChart = ({ trial, trains }: any) => {
    ...
    const handleSocketMessage = useCallback((e: MessageEvent<any>) => {
        try {
            ...
            refineChartData(msg, chartRefAccuracy)
        } catch (e) {
            logger.error(e)
        }
    }, [trains, chartRefAccuracy])
    ...
    useEffect(() => {
        ...
        initChartData([chartRefAccuracy], [t(`metric.${trial.target_metric}`)])
        initAnnotation([accuracyOptions])
    ...
    return (
        <Col>
            ...
            <Row>
                <ButtonArea>
                    <Button variant='info' onClick={() => onResetZoom(chartRefAccuracy.current)}>
                        <i className='fas fa-sync' /> {t(`button.zoom.init`)}
                    </Button>
                    <Button variant='info' onClick={() => onZoomIn(chartRefAccuracy.current)}>
                        <i className='fas fa-plus' /> {t(`button.zoom.in`)}
                    </Button>
                    <Button variant='info' onClick={() => onZoomOut(chartRefAccuracy.current)}>
...
export default VisionADTrainChart