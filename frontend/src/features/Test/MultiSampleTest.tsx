...
import { RadioGPUWithCPU, SelectDataModal, TableClsTrialDetail, emptyTestMultiSampleErrors, validate } from 'features'
import { LabelSelect2 } from 'components'


const MultiSampleTest = forwardRef(({ toggle, selectedTrial, selectedModels, engineType }: any, ref) => {
    ...
    useEffect(() => {
        ...
        if (selectedTrial) {
            ...
            if (selectedModels.uuid && selectedModels.name) {
                ...
                const label = engineType === engine.table_cls ?
                    `Trial${selectedModels.train_id}_${selectedModels.name} [${t(`metric.${selectedModels.target_metric.String}`)}: ${(parseFloat(selectedModels.score.String.replaceAll("\"", "")) * 100).toFixed(2)}%]`
                    : `Trial${selectedModels.train_local_id}_${selectedModels.name} [${t(`metric.${selectedModels.target_metric.String}`)}: ${(Number(selectedModels.score.String).toFixed(3))}]`
    ...
    const handleSubmit = () => {
        ...
        const errors = validate(newRequestData, t)
...
export default MultiSampleTest